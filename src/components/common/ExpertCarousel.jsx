  ...
  return (
    ...
    >
      <div className="flex items-start gap-2 md:gap-3 mb-2 md:mb-3">
        <div
          className={`${
            featured ? "w-14 h-14 md:w-20 md:h-20" : "w-12 h-12 md:w-14 md:h-14"
          } rounded-full overflow-hidden flex-shrink-0 border-2 border-amber-100 transition-transform duration-500 group-hover:scale-110 group-hover:rotate-3`}
        >
          <img src={image} alt={name} className="w-full h-full object-cover" />
        </div>
        <div className="flex-1 min-w-0">
          <h3
            className={`font-bold text-gray-800 transition-colors duration-300 group-hover:text-blue-400 ${
              featured ? "text-base md:text-lg" : "text-sm md:text-base"
            } mb-0.5 md:mb-1 truncate`}
          >
            {name}
          </h3>
          <div className="flex items-center gap-3 mb-1">
            <Briefcase className="w-3 h-3 md:w-5 md:h-5 text-amber-600 shrink-0" />
            <span className="text-[10px] md:text-xs font-medium text-amber-700 uppercase tracking-wider">
              {title}
            </span>
          </div>
        </div>
      ...
      <p className="text-xs md:text-sm text-gray-600 leading-relaxed flex-1 line-clamp-3 md:line-clamp-4 transition-colors duration-300 group-hover:text-gray-900">
        {description}
      </p>
      ...
      >
        ...
        {featured ? (
          <>
            <Video className="w-5 h-5 transition-transform group-hover/btn:scale-110" />
