...
import QuotesSection from "../../components/common/QuotesSection";
...
import InterviewsSection from "../../components/common/InterviewsSection";
...
export default function PhanTichGocNhin() {
  ...
  return (
    <div className="min-h-screen bg-[#f6eadf] text-gray-800 relative overflow-x-hidden">
      ...
      {/* Main Content */}
      <main className="container mx-auto max-w-7xl px-8 md:px-12 py-12 space-y-22">
        {/* Articles Section */}
        <ScrollReveal>
          <ArticlesSection />
        </ScrollReveal>
      ...
      {showBackToTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 z-50 flex items-center justify-center bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white p-4 rounded-full shadow-2xl hover:shadow-amber-400/50 transition-all duration-300 hover:scale-110 group animate-in slide-in-from-bottom fade-in"
        >
          <ArrowUp className="w-6 h-6 transition-transform duration-300 group-hover:-translate-y-2" />
        </button>
      )}
    </div>
    ...
    //     style={{
    //       backgroundImage:
    //         'linear-gradient(180deg, rgba(0,0,0,0.7), rgba(0,0,0,0.8)), url("/images/analysis-bg.jpg")',
    //     }}
    //   >
    ...
    //         Nơi hội tụ các phân tích chuyên sâu và góc nhìn đa chiều về di sản,
    //         nghệ thuật và công nghệ — giúp độc giả tiếp cận thông tin một cách
    ...
    //             <i
    //               className="fa-solid fa-star animate-spin"
